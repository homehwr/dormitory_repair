<template>
  <div class="repair-detail-container" v-loading="loading">
    ...
    <div class="content-wrapper">
      <!-- 状态卡片 -->
      <div class="status-card" @click="changeButton">
      ...
      <el-dialog title="修改状态" :visible.sync="dialogVisible" width="80%" custom-class="dialog">
        <el-button type="warning" class="changeStatusButton" @click="changeStatus(repair_info.id,0)">待维修</el-button>
        <el-button type="success" class="changeStatusButton" @click="changeStatus(repair_info.id,1)">已维修</el-button>
        <el-button type="primary" class="changeStatusButton buttonClass1" @click="changeStatus(repair_info.id,3)">已转服务商</el-button>
      </el-dialog>
    </div>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      statusIcon: '',
      statusClass: '',
      phone: localStorage.getItem('dormitory_account'),
      dialogVisible: false
    }
  },
  async mounted() {
...
.cancel-button {
  ...
  font-weight: 500;
  border-radius: 12px;
  background: linear-gradient(to right, #1482ef, #409eff);
  border: none;
